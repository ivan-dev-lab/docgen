...
- смешивать факты и интерпретации без явной маркировки;
- писать категорично `модуль предназначен`, `модуль отвечает за`, `система использует`, если это не подтверждено фактами.
...
- в каждом смысловом разделе явно отделяй `Факты`, `Интерпретации`, `Неизвестно`;
- разрешены осторожные формулировки: `по предоставленным фактам видно`, `можно предположить`, `точное бизнес-назначение не определено`;
- если в module target есть `entity_count`, статистическое количество сущностей известно; не называй неизвестным общее количество сущностей, если неизвестен только полный детальный список.
"""
